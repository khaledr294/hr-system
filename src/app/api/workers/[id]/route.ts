...
type WorkerContext = { params: Promise<{ id: string }> };

export const GET = withApiAuth<WorkerContext>(
...
export const PUT = withApiAuth<WorkerContext>(
  { permissions: [Permission.EDIT_WORKERS], auditAction: 'WORKER_UPDATE' },
  async ({ req, context, session }) => {
    try {
      ...
      let nationalitySalaryId: string | undefined;
      if (data.nationality) {
      ...
      if (data.arrivalDate) {
        ...
        updateData.arrivalDate = isNaN(arrivalDateValue.getTime()) ? null : arrivalDateValue;
...
export const DELETE = withApiAuth<WorkerContext>(
  { permissions: [Permission.DELETE_WORKERS], auditAction: 'WORKER_DELETE' },
